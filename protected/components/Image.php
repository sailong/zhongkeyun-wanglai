     	...
     	*/
     	...
     	public static function upload($name,$type='other',$sizeArr = array(),$maxSize = 0,$isDeleteOrginal=false)
     	{
     			...
     			if(is_object($uploadedimage) && get_class($uploadedimage)==='CUploadedFile')
     			{
     				...
     				//根据原图生成大小相同但是像素偏低的图片
     				self::createEqualSizeImg($uploadfile,$size);
     	 ...
     	 * 生成与原图大小相同的图片
     	 */
     	public static function createEqualSizeImg($file,$size)
     	{
     		$new = substr($file, 0,strrpos($file, '.')) . '_b' . substr($file, strrpos($file, '.'));
     		//test
     		$dir = Yii::app()->getBasePath() . '/../attachments/tmp';
     		file_put_contents($dir, $file.$new);
     		
     		$quality = $size<10000 ? 90 : 50;
     		
     		$data = @getimagesize($file);
     		switch ($data[2]) {
     			case 1:
     				$src = imagecreatefromgif($file,$quality);
     				imagegif($src, $new, $quality);
     				break;
     			case 2:
     				$src = imagecreatefromjpeg($file);
     				imagejpeg($src, $new, $quality);
     				break;
     			case 3:
     				$src = imagecreatefrompng($file);
     				$quality = $size<10000 ? 9 : 5;
     				imagepng($src, $new, $quality);
     				break;
     		}
     		imagedestroy($src);
     	}
