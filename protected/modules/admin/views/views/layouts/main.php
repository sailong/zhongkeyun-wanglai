	...
	<div class="main">
		<div class="menu">
			...
			foreach ($menuList as $m)
			{
				...
				$id = strtolower($m['id']);
			?>
			    <li <?php if($controller_id == $id) { ?> class="cur" <?php } ?> ><a href="<?php echo Yii::app()->createUrl('/admin/'.$m['id'])?>">+<?php echo $m['name'];?></a></li>
			<?php 
			}
			if($isManage)
			{
			?>
			<li  <?php if($controller_id=='user') { ?> class="cur" <?php } ?>><a href="/index.php?r=admin/User">+系统用户管理</a></li>
			<?php }?>
